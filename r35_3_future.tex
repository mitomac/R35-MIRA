...
%We will continue our collaboration with Stephen Bell (MIT) to id



%We will attempt to block initiation of DNA replication by using temperature sensitive mutants mutants of key initiation factors (eg. primase or mcm10). We will use our genome-wide chromatin occupancy profiling to assess chromatin architecture in a factor agnostic manner. Alternatively, we can explore degrons or anchors away if the arrest is not tight enough.   ChIP-exo of key components. dominant negatives. fob1. e
...
%We will use CRISPR/Cas9 to generate degron AID tagged alleles of ORC in Drosophila. We will deplete cells of ORC in G2 arrested cells and 



%We've found that Mcm2-7 loading occurs in two steps a cylcin E/cdk independent step where limiting amounts of Mcm are loaded at ORC binding sites, and a cyclin E/cdk2 dependent step whereby 40-fold more Mcm2-7 is loaded and re-distributed away from ORC binding sites. If ORC is able to translocate through the chromatin enviroment, we  
...
%[while its beenknown that ORC is not essential for activation -- only loadigin]

%We will first directly assess the role of transcription  shaping the distribution of the Mcm2-7 complex.  We will use CRISPR/Cas9 genomic editing CRISPR/dCas9 and d transcription -- crispr mediated disruption of txp and assess Mcm loading in intragenic region. If it is not M 
%Models for mcm loading
%1. translocate away from ORC
%2. load at distance from ORC via chromatin looping and 3C interactions -- supported by domains.
%3. Transient ORC interactions that are sufficient to load mcm.  



%Mcm either translocates away from the ORC binding site or it is
%What is the mechanism by which the Mcm complex is distributed ? translocation through the chromatin?  limit loading translocation...orc bound or not?





%[ gene editing or targeting.  

We will also continue our ongoing collaboration with the Duronio group at UNC to investigate the role of specific chromatin marks in regulating replication timing, genome stability and origin usage in \dros\citep{Li2016-fi}(Letter from Duronio).  The Duronio group has developed a genetic histone replacement strategy for the fly\citep{McKay2015-nn}, allowing us to directly test the impact of specific histone tail mutants on DNA replication in proliferating tissues (\eg imaginal discs).  Unlike manipulation of chromatin modifying enzymes, this approach eliminates potential off target actions and provides an unbiased assessment of the role of specific chromatin modifications in regulating the DNA replication program.
...
% Interestingly, the Mcm2-7 complex is able to translocate on DNA in vitro and in both fly and yeast, it's distribution is shaped by transcription. 

 

%How are origins specified in Drosophila?  points of intiation -- broad correlations -- remarkbaly plastic -- hyrien paper --  So what shape origin usage?  We propose that the Mcm2-7 complex is not precisely located and is redistributed throughout the genome  
...
%Specifically, we will  pulse label cells for 2.5 minutes with the nucleoside analog, EdU, which will allow us to enrich for nascent chromatin associated with the replication fork.  Similarly, by chasing the EdU pulse for increasing lengths of time, we will be able to enrich for chromatin as it matures behind the fork.   Importantly, the proposed assay monitors the incorporation of EdU directly at the fork and as it matures behind the fork, thus allowing the kinetics of chromatin maturation to be followed in an asynchronous population of cells.  As a proof of principle, we have generated chromatin occupancy maps for nascent (2.5 minute EdU pulse, 0 minute chase), intermediate (2.5 minute EdU pulse, 10 minute chase) and mature (2.5 minute EdU pulse, 60 minute chase) chromatin.  We find that immediately following fork passage there is very little chromatin organization, but 10 minutes after passage of the fork we can start to see the chromatin organization that resembles mature chromatin including the recruitment of DNA binding factors.  We expect to address multiple fundamental questions in chromatin biology.  For example, are there locus-specific differences in chromatin maturation - perhaps dependent on the local chromatin state or time of replication during S-phase? Is there a difference in replication or transcription dependent chromatin organization?  Do all DNA-binding factors re-associate with the chromatin with the same kinetics, or are there factor-specific differences?  For specific DNA-binding factors, are there locus specific differences in recruitment behind the fork?  As this work matures we will interrogate the role of histone chaperones and ATP-dependent chromatin remodelers in the re-establishment of chromatin organization behind the replication fork.  
...
%\citep{Rogakou1999,Vignard2013,Lee2005,Lee2007,Symington2011,Burma2001,Burma2001a,Berkovich2007,Nakada2003,Harrison2006,Kurz2004,Haber2016,Celeste2003,Lee2004,Downs2000,Foster2005,Shechter2004,Wolner2003,Stracker2004}. 
In addition to the factors required for break recognition, processing and repair via homologous recombination or non-homologous end joining, the local chromatin environment also influences the accessibility of the break site%\citep{Tsukuda2005}
%\cite{Tsukuda2005,Clapier2009,Chai2005,Mizuguchi2004,Morrison2004,Alexeev2003,Chai2005a,Kwon2015,Kwon2014,Berkovich2007,Shroff2004,Ataian2006,Liang2007,Harrison2006,Tsabar2016,Morillo-Huesca2010} 
and the kinetics of recognition and repair\citep{Price2013-lp,Smerdon1991-sv}.
%\citep{Tsukuda2009,Tsabar2016,Lee2014}.
...
%\cite{Celeste2003,Tsukuda2005,Moore2007,Foster2005,Symington2011,Iacovoni2010,Kim2007}.
%Further, there are also global changes in epigenetic state and nuclear architecture that occur in a cell burdened with DSBs\cite{Hauer2017}.   Although prior experiments have clearly shown a loss of nucleosome occupancy surrounding the break at the \textit{MAT} locus\citep{Tsukuda2005} and in mammalian cells\citep{Berkovich2007}, they 
Prior experiments have lacked the temporal and spatial resolution to observe %subtle changes in chromatin organization
...
%Because of its endogenous cutting efficiency and biochemical/molecular convenience, many seminal DSB experiments in yeast have primarily investigated HO endonuclease induced DSBs at the mating type \textit{MAT} locus\cite{Haber2012}. All studies have noted a profound loss of nucleosome occupancy following break recognition by the Mre11-Rad50-Xrs2 complex (MRX), which is a conserved phenomena observed in mammalian systems\cite{Goldstein2013}; %Then, following determination of repair pathway choice, an accumulation of appropriate repair factor binding at this locus\cite{Symington2011,Lisby2004,Hicks2011,Haber2016,Seeber2017,Tsukuda2005,Harrison2006,Haber2012};
%Although prior experiments have clearly shown a loss of nucleosome occupancy surrounding the break at the \textit{MAT} locus\citep{} and in mammalian cells\citep{}, they have all lacked the temporal and spatial resolution to observe subtle changes in chromatin organization and discern the precise mechanisms by which nucleosome organization and occupancy is lost (\eg eviction or sliding of nucleosomes?) and the fate of bound transcription factors.  We will use our GCOP assay to precisely profile the temporal cascade of chromatin structure changes that occur surrounding site-specific double strand breaks and their subsequent repair via NHEJ or HR.


%and are often confounded by strand resectioning.
%While DSBs can be sporadically induced with irradiation or site-specifically induced with nuclease targeting strategies at engineered loci, these systems have lacked either the temporal or spatial resolution to discern the precise mechanisms by which nucleosome organization and occupancy is lost (\eg eviction or sliding?) following break induction and ultimately restored following repair.  We will use our GCOP assay to profile the temporal cascade of chromatin structure changes that occur surrounding site-specific double strand breaks and their subsequent repair via NHEJ or HR.

%Also as most  or if these mechanisms are conserved across diverse loci with different chromatin states.  %in regions the locus-specificity and break induction synchrony at diverse chromatic regions to determine if these  a universal phenomena.
%In addition, it is unclear how dependent the observed changes in nucleosome occupancy are on the local chromatin structure and state of the \textit{MAT} locus.
...
\ssheading{Dynamic perturbations of chromatin structure at site specific DSBs}
In collaboration with the Haber laboratory (Haber letter) we have used markerless CRISPR/Cas9\cite{Dekker2017} to introduce multiple ectopic HO-endonuclease recognition sequences into a half dozen chromatically privileged loci (TF bound regions, centromeres, telomeres, etc.) in a single GAL::HO yeast strain.  %Site specific DSBs at these ectopic loci is rapid and dependent on GAL::HO induction.% and is rapid with 90\% of the DNA being cut within 15-30 minutes of galactose addition.  
...
A more complicated challenge is to assess chromatin changes that occur during and after repair by HR as %PCR and genomic-based approaches to study nucleosome occupancy lack the ability 
...
%or were conducted at the \textit{MAT} locus which has inherently poorly positioned nucleosomes\citep{Weiss1998-lq}.
We propose to discriminate between the donor and break template by specifically labeling either the ectopic donor or break template with the nucleoside analogue, EdU ({\color{dukeblue}\textbf{Figure 6}}). Sequences proximal ($<$15 kb) to early origins can be specifically labeled %to early   We and others have previously demonstrated that it is possible to specifically label only those sequences proximal ($<$15 kb) to early replicating origins, 
by releasing cells from a G1 $\alpha$-factor arrest into hydroxyurea (HU) in the presence of BrdU or EdU\citep{Peace2016-rb,Belsky2015-li}.  Importantly, these cells can be released back into the cell cycle and will retain the early origin-specific labeling for several generations. % albeit at diluted levels.  
...
\pagebreak
\bibliography{paperpile,vinay_dna-damage}
